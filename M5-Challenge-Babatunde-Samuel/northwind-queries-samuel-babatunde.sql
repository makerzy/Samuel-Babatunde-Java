...
select e.first_name, e.last_name  from employees e where e.last_name like 'W%';


-- 5. List all customers from zip codes that start with 55.
select * from customers c where c.postal_code like '55%';

-- 6. List all customers from zip codes that end with 0.
select * from customers c where c.postal_code like '%0';


-- 7. List the first name, last name, and email for all customers with a ".org" email address.
select c.first_name, c.last_name, c.email  from customers c where c.email  like '%.org';

-- 8. List the first name, last name, and phone number for all customers from the 202 area code.
select c.first_name , c.last_name , c.phone  from customers c where c.phone like '1-(202)%';
...
select c.first_name , c.last_name , c.phone  from customers c where c.phone like '1-(202)%' order by c.last_name, c.first_name asc;
